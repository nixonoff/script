...
function main()
    ...
    while true do
		...
		if not sampIsChatInputActive() and not sampIsDialogActive() and not isSampfuncsConsoleActive() and sp_active == true and isKeyDown(VK_MENU) and isKeyJustPressed(VK_1) then
...
function imgui.OnDrawFrame()
	...
	if ban.v then
		...
		imgui.Begin(u8"Забанить", ban)
			if imgui.Button(u8"Чит", btn_size) then
				sampSendChat("./ban "..id_buffer.v.." 10 Чит")
				ban.v = not ban.v
			end
			if imgui.Button(u8"Бот", btn_size) then
				sampSendChat("./ban "..id_buffer.v.." 7 Бот")
				ban.v = not ban.v
			end
			if imgui.Button(u8"Аим", btn_size) then
				sampSendChat("./ban "..id_buffer.v.." 15 Аим")
				ban.v = not ban.v
			end
			if imgui.Button(u8"ГМ", btn_size) then
				sampSendChat("./ban "..id_buffer.v.." 15 ГМ")
				ban.v = not ban.v
			end
			if imgui.Button(u8"ВХ", btn_size) then
				sampSendChat("./ban "..id_buffer.v.." 15 ВХ")
				ban.v = not ban.v
			end
			if imgui.Button(u8"Оскорбление родни", btn_size) then
				sampSendChat("./ban "..id_buffer.v.." 15 Оск. род.")
				ban.v = not ban.v
			end
		imgui.End()
	end
	if jail.v then
		...
		imgui.Begin(u8"Посадить в тюрьму", jail)
			if imgui.Button(u8"ДМ", btn_size) then
				sampSendChat("./jail "..id_buffer.v.." 30 Дм")
				jail.v = not jail.v
			end
			if imgui.Button(u8"ДМ в ЗЗ", btn_size) then
				sampSendChat("./jail "..id_buffer.v.." 40 Дм в зз")
				jail.v = not jail.v
			end
			if imgui.Button(u8"Сбив переката", btn_size) then
				sampSendChat("./jail "..id_buffer.v.." 40 Сбив переката")
				jail.v = not jail.v
			end
			if imgui.Button(u8"Сбив", btn_size) then
				sampSendChat("./jail "..id_buffer.v.." 40 Сбив")
				jail.v = not jail.v
			end
		imgui.End()
	end
	if mute.v then
		...
		imgui.Begin(u8"Выдать мут", mute)
			if imgui.Button(u8"Нецензурная лексика", btn_size) then
				sampSendChat("./mute "..id_buffer.v.." 15 Нецензурная лексика")
				mute.v = not mute.v
			end
			if imgui.Button(u8"Оскорбления", btn_size) then
				sampSendChat("./mute "..id_buffer.v.." 20 Оск")
				mute.v = not mute.v
			end
			if imgui.Button(u8"Неадекватность", btn_size) then
				sampSendChat("./mute "..id_buffer.v.." 30 Неадекват")
				mute.v = not mute.v
			end
			if imgui.Button(u8"Капс", btn_size) then
				sampSendChat("./mute "..id_buffer.v.." 10 Капс")
				mute.v = not mute.v
			end
			if imgui.Button(u8"Флуд", btn_size) then
				sampSendChat("./mute "..id_buffer.v.." 10 Флуд")
				mute.v = not mute.v
			end
			if imgui.Button(u8"Упоминание родни", btn_size) then
				sampSendChat("./mute "..id_buffer.v.." 300 Упом. род.")
				mute.v = not mute.v
			end
		imgui.End()
	end
	if kick.v then
		...
		imgui.Begin(u8"Кикнуть", kick)
			if imgui.Button(u8"ДМ", btn_size) then
				sampSendChat("./kick "..id_buffer.v.." Дм")
				kick.v = not kick.v
			end
			if imgui.Button(u8"ДМ кар", btn_size) then
				sampSendChat("./kick "..id_buffer.v.." Дм кар")
				kick.v = not kick.v
			end
			if imgui.Button(u8"Помеха", btn_size) then
				sampSendChat("./kick "..id_buffer.v.." Помеха")
				kick.v = not kick.v
			end
			if imgui.Button(u8"НонРП", btn_size) then
				sampSendChat("./kick "..id_buffer.v.." НонРП")
				kick.v = not kick.v
			end
			if imgui.Button(u8"Багоюз", btn_size) then
				sampSendChat("./kick "..id_buffer.v.." Багоюз")
				kick.v = not kick.v
			end
		imgui.End()
	end
	if warn.v then
		...
		if imgui.Button(u8"ДМ", btn_size) then
			sampSendChat("./warn "..id_buffer.v.." Дм")
			warn.v = not warn.v
		end
		if imgui.Button(u8"Сбив", btn_size) then
			sampSendChat("./warn "..id_buffer.v.." Сбив")
			warn.v = not warn.v
		end
		if imgui.Button(u8"Сбив переката", btn_size) then
			sampSendChat("./warn "..id_buffer.v.." Сбив переката")
			warn.v = not warn.v
		end
		if imgui.Button(u8"Сбив аптеки", btn_size) then
			sampSendChat("./warn "..id_buffer.v.." Сбив аптеки")
			warn.v = not warn.v
		end
		if imgui.Button(u8"+С", btn_size) then
			sampSendChat("./warn "..id_buffer.v.." +С")
			warn.v = not warn.v
		end
		if imgui.Button(u8"Отвод", btn_size) then
			sampSendChat("./warn "..id_buffer.v.." Отвод")
			warn.v = not warn.v
		end
		if imgui.Button(u8"Багоюз", btn_size) then
			sampSendChat("./warn "..id_buffer.v.." Багоюз")
			warn.v = not warn.v
		end
		imgui.End()
	end
